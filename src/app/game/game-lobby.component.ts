...
import { copyOutline, qrCodeOutline, closeOutline, personOutline } from 'ionicons/icons'
  ...
  template: `
    ...
    <ion-content [fullscreen]="true" class="lobby-content">
      ...
      <div class="code-banner">
        ...
        <ion-button fill="clear" size="small" class="qr-toggle" (click)="showQr.set(!showQr())">
          <ion-icon [name]="showQr() ? 'close-outline' : 'qr-code-outline'" slot="start"></ion-icon>
          ...
          <div class="players-grid">
            @for (player of players(); track player.uid) {
              <div class="player-chip">
                <div class="player-avatar">{{ player.alias[0].toUpperCase() }}</div>
...
export class GameLobbyComponent {
  ...
  async copyCode() {
    ...
    ;(await this.toastCtrl.create({ message: '✓ Code copied!', duration: 1500, color: 'success', position: 'top' })).present()
  }
}
